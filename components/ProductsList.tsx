	...
	return (
		<>
			<div
				className={`${
					data.length > 0 && "grid"
				} grid-cols-6 gap-6 px-4 py-8 max-xl:grid-cols-5 max-lg:grid-cols-4 max-md:grid-cols-3 max-sm:grid-cols-2 max-xs:grid-cols-1 max-xs:text-xs`}
			>
				{data.length > 0 ? (
					data.map(
						(product, index) =>
					...
					<h3 className="text-center p-4 text-zinc-400 text-lg">
